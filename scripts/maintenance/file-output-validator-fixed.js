 ...
 * - 提供修复建议
 * 
 * 用法：node scripts/maintenance/file-output-validator-fixed.js [options]
 */
...
return issues;
    ...
    for (const file of files) {
        try {
            ...
            if (this.options.verbose && !controlChars) {
                console.log(`  ✅ ${file}`);
            }
        } catch (error) {
    ...
    for (const file of files) {
        try {
            ...
            if (this.options.verbose && !hardcodedPaths) {
                console.log(`  ✅ ${file}`);
            }
        } catch (error) {
...
用法: node file-output-validator-fixed.js [选项]
...
示例:
  node file-output-validator-fixed.js
  node file-output-validator-fixed.js --auto-fix --verbose
  node file-output-validator-fixed.js --directories=src/ --extensions=.ts
        `);
}
}
